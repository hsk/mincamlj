...
object Simm13 extends X86Asm {
	...
	def h(e:Fundef):Fundef = e match {
		case Fundef(l, xs, ys, e, t) => Fundef(l, xs, ys, g(M.empty(), e), t)
	}

	// プログラム全体の13bit即値最適化
	def f (e:Prog):Prog = e match {
		case Prog(data, fundefs, e) => Prog(data, fundefs.map(h) , g(M.empty(), e))
	}
}
